...
GRAD_CLIP=1
# model config
CHECKPOINT_DIR=/gs/bs/tge-gc24sp03/combined_checkpoints/idefics2-siglip-so400m-patch14-384-Codestral-22B-v0.1
CHECKPOINT_SAVE_DIR=/gs/bs/tge-gc24sp03/checkpoints/idefics2-siglip-codestral-22b/tikz-local-step1/LR${LR}-MINLR${MIN_LR}-WD${WEIGHT_DECAY}-GC${GRAD_CLIP}-BS${GLOBAL_BATCH_SIZE}-loss-mask-step1-all
...
mpirun -np $NUM_GPUS \
  ...
  --eval-iters 10 \
  --vocab-size 32771 \
  --vlm-text-hidden-size 6144 \
  --vlm-text-intermediate-size 16384 \
  --vlm-text-num-attention-heads 48 \
  --vlm-text-num-hidden-layers 56 \
